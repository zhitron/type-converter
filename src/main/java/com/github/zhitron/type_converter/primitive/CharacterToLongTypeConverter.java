 ...
 * 字符到长整型的类型转换器
 * <p>
 * 该转换器用于将Character类型转换为Long类型，通过将字符的数值直接转换为long类型实现。
 * 这是一个单例类，通过INSTANCE字段获取实例。
 * </p>
 *
 * @author zhitron
 */
public class CharacterToLongTypeConverter extends TypeConverter<Character, Long> {
    /**
     * 单例实例
     */
     ...
     */
    protected CharacterToLongTypeConverter() {
        super(Character.class, Long.class);
    }
     ...
     */
    @Override
    public Long convertsUnchecked(Character source) throws Throwable {
        return (long) source;
    }
}
