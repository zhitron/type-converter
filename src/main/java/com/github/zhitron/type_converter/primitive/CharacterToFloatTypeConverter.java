 ...
 * 字符到浮点数的类型转换器
 * <p>
 * 该转换器用于将Character类型转换为Float类型，通过将字符的数值直接转换为float类型实现。
 * 这是一个单例类，通过INSTANCE字段获取实例。
 * </p>
 *
 * @author zhitron
 */
public class CharacterToFloatTypeConverter extends TypeConverter<Character, Float> {
    /**
     * 单例实例
     */
     ...
     */
    protected CharacterToFloatTypeConverter() {
        super(Character.class, Float.class);
    }
     ...
     */
    @Override
    public Float convertsUnchecked(Character source) throws Throwable {
        return (float) source;
    }
}
