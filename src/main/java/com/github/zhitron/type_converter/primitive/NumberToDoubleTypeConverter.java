 ...
 * 数字到双精度浮点数的类型转换器
 * <p>
 * 该转换器用于将Number类型转换为Double类型，直接调用Number的doubleValue()方法进行转换。
 * 这是一个单例类，通过INSTANCE字段获取实例。
 * </p>
 *
 * @author zhitron
 */
public class NumberToDoubleTypeConverter extends TypeConverter<Number, Double> {
    /**
     * 单例实例
     */
     ...
     */
    protected NumberToDoubleTypeConverter() {
        super(Double.class);
    }
     ...
     * 直接调用Number.doubleValue()方法获取双精度浮点数值
     *
     ...
     */
    @Override
    public Double convertsUnchecked(Number source) throws Throwable {
